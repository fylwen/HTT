...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        if self.return_inter:
            intermediates["res_conv1_relu"] = x.cpu()

        x = self.maxpool(x)

        x = self.layer1(x)
        if self.return_inter:
            intermediates["res_layer1"] = x.cpu()
        x = self.layer2(x)
        if self.return_inter:
            intermediates["res_layer2"] = x.cpu()
        x = self.layer3(x)

        #res_layer4=x
        if self.return_inter:
            intermediates["res_layer3"] = x.cpu()
        x = self.layer4(x)

        res_layer5=x
        if self.return_inter:
            intermediates["res_layer4"] = x.cpu()
